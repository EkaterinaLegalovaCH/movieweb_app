...
class JSONDataManager(DataManagerInterface):
    ...
    def add_user_movie(self, user_id, title):
        ...
        new_rating = rating
        new_poster = img_url
        new_movie = {'name': new_title, 'director': new_director, 'year': new_year, 'rating': new_rating, "poster": new_poster}
    ...
    def update_user_movie(self, user_id, movie_id):
        with open(self.filename, "r") as handle:
            ...
            users_dict = json.loads(users_data)
        user_movies = users_dict[user_id]['movies']
        new_title = input("Write a title: ")
        new_director = input("Write a director: ")
        new_year = input("Write a year: ")
        new_rating = input("Write a rating: ")
        user_movies[movie_id] = {'name': new_title, 'director': new_director, 'year': new_year, 'rating': new_rating}
        users_dict[user_id]['movies'] = user_movies
        new_data = json.dumps(users_dict)
        with open(self.filename, "w") as handle:
            handle.write(new_data)
