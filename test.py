...
def test_delete_user():
    response = client.post('/users/delete_user/5')
    assert response.status_code == 302
...
def test_update_movie():
    response = client.post('/users/1/update_movie/4', data={"name": "Test Movie", "director": "Director", "year": "Year", "rating": "10", "poster": "Some"})
    assert response.status_code == 302


def test_update_movie_form():
    response = client.get('/users/1/update_movie_form/4')
    assert response.status_code == 200
    assert b"<title>Update Movie Form</title>" in response.data


def test_delete_movie():
    response = client.post('/users/1/delete_movie/4')
    assert response.status_code == 302
